...
public class UTIL {
	...
	public static byte[] FillArray(byte[] data,String mode){
		...
		if (nn != 8) {
			byte[] tbs = new byte[nn];
			for (int i = 0; i < nn; i++){
				if(mode.equals(FILLWITHSPACE))tbs[i] = 0x20;
				if(mode.equals(FILLWITHNULL))tbs[i] = 0x00;
				}
			return (union(data,tbs));
		}else{
			return data;
	...
	public static String GetHexData(byte bytes[]){
		StringBuffer content = new StringBuffer();
		for(int i = 0;i< bytes.length;i++)content.append(bytesToHexStr(bytes[i]));
		return "";
	}
	
	/**
	 * GetHexData
	 * 获取bytes数组偏移量offset后的len长度的hex 0xab -->ab的值
	 * */
	...
	public static String backData(Map<String,String> map){
		...
		while(keyit.hasNext()){
			...
			/*if(!(key.equals("sign")) && !(key.equals("sign_type")) && !(key.equals("is_success"))){
				keylis.add(key);
			}*/
			keylis.add(key);
		}
	...
	public static String hexStringTo10String(String hexString) {
		try {
